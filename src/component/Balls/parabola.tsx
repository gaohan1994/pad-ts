...
export default class Parabola {
  ...
  private target: any;
  private options: any;
  private flagMove: any;
  private params: any;
  private coordTarget: any;
  private stopRun: any;
  private moveStyle: any;
  ...
  stop(stopCompleteFn: boolean = true) {
    this.stopRun = true;
    // 清除结束运动后的回调函数

    if (stopCompleteFn) {
      this.params.complete = null;
    }
  }

  getParams() {
    const { options, defaults, params } = this;
  ...
  curvilinear() {
    ...
    const { params, moveStyle } = this;

    const { element, target } = that;

    let { coordTarget } = this;
    const targetIsEle = typeof (target.sx) === 'undefined';
